<x-layouts.sidebar-link href="{{ route('admin.dashboard') }}" :active="request()->routeIs('admin.dashboard')">
    <svg class="size-5 mr-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
...
<x-layouts.sidebar-link href="{{ route('admin.menu.index') }}" :active="request()->routeIs('admin.menu.*')">
...
<x-layouts.sidebar-link href="{{ route('admin.orders.index') }}" :active="request()->routeIs('admin.orders.*')">
    <div class="flex items-center justify-between w-full">
        ...
        @php
            $pendingOrdersCount = \App\Models\Order::where('status', 'menunggu')->count();
        @endphp
        @if($pendingOrdersCount > 0)
            <span class="inline-flex items-center justify-center size-5 rounded-full text-[10px] font-bold bg-amber-500 text-white animate-pulse shadow-2xs">
                {{ $pendingOrdersCount }}
            </span>
        @endif
    </div>
...
<x-layouts.sidebar-link href="{{ route('admin.kurir.index') }}" :active="request()->routeIs('admin.kurir.*')">
    <svg class="size-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<x-layouts.sidebar-link href="{{ route('admin.verifikasi.index') }}" :active="request()->routeIs('admin.verifikasi.*')">
    <div class="flex items-center justify-between w-full">
        <div class="flex items-center">
            <svg class="size-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @php
            $pendingVerifikasiCount = \App\Models\User::where('role', 'siswa')
                ->where('is_verified', false)
                ->whereNotNull('kartu_pelajar_photo')
                ->count();
        @endphp
...
<x-layouts.sidebar-link href="{{ route('admin.reports.index') }}" :active="request()->routeIs('admin.reports.*')">
    <svg class="size-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<x-layouts.sidebar-link href="{{ route('admin.settings.index') }}" :active="request()->routeIs('admin.settings.*')">
    <svg class="size-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
